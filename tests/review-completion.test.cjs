...
function setup(tabs={}) {
  ...
  const c=createSheetReadContext({getNamedSheet_:(ss,name)=>ss.getSheetByName(name),getRubricStructure_:()=>Object.fromEntries(['review1','custom_check'].map(key=>[key,Array.from({length:7},(_,i)=>({pi:'PI'+(i+1),name:'Criterion',co:'CO1',maxMarks:i===6?10:15,type:'Team'}))])),Logger:{log(){}},SHEET_NAMES:{TEAM_STATUS:'teams',REVIEW_COMMITTEE:'committees'},FIELD_DEFINITIONS:{TEAM_STATUS:{},REVIEW_COMMITTEE:{}},
...
require('./milestone-fixture.cjs').install(c);
  // Legacy level-sheet behavior is retained for custom reviews only.
  c.definitions=c.definitions.map(r=>r.key==='review2'?{...r,key:'custom_check',label:'Custom Check'}:r);
...
test('third configured review participates in completion and student marks',()=>{
 ...
 f.c.setReviews(3);
 const structure=f.c.getRubricStructure_();structure.review3=structure.review1;structure.review2=structure.review1;
...
test('bulk and single-team results agree; each spreadsheet opens once and tab reads once',()=>{
  const tabs={'Review1Evaluations':[row('T1','A',zero()),row('T1','B',zero()),row('T2','C',Array(7).fill('')),row('T3','D',zero())],
    'Committee 1 - custom_check':[row('T1','A',zero()),row('T1','B',[0,0,0,0,0,0,''])]};
  const bulk=setup(tabs);const all=bulk.c.getAllReviewCompletionStatus_();
  assert.equal(all.t1.review1.completed,false);assert.equal(all.t1.custom_check.completed,false);
...
test('duplicate and foreign student rows cannot falsely complete a team',()=>{
  const {c}=setup({'Committee 1 - custom_check':[row('T1','A',zero()),row('T1','A',zero()),row('T1','B',zero()),row('T1','stranger',zero())]});
  const result=c.getTeamReviewCompletionStatus_('T1').custom_check;
...
test('a failed review does not prevent the next review from loading',()=>{
  const {c}=setup({'Review1Evaluations':Error('unavailable'),'Committee 1 - custom_check':[row('T1','A',zero()),row('T1','B',zero())]});
  const result=c.getTeamReviewCompletionStatus_('T1');
  assert.equal(result.review1.completed,false);assert.equal(result.custom_check.completed,true);
  assert.equal(result.review1.available,false);assert.equal(result.custom_check.available,true);
});

test('missing tabs are unavailable while a readable empty tab is incomplete',()=>{
  const {c}=setup({'Committee 1 - custom_check':[]});
  const result=c.getTeamReviewCompletionStatus_('T1');
  assert.equal(result.review1.available,false);
  assert.equal(result.custom_check.available,true);
  assert.equal(result.custom_check.completed,false);
});

test('student totals above 5 and all-zero completed assessments are preserved',()=>{
  const {c}=setup({'Review1Evaluations':[row('T1','A',Array(7).fill(4),80)],'Committee 1 - custom_check':[row('T1','A',zero(),0)]});
  ...
  const high=setup({'Committee 1 - custom_check':[row('T1','A',Array(7).fill(4),80)]}).c.getStudentAllReviewMarks('a@example.com');
  assert.equal(high.custom_check.totalMarks,80);assert.equal(high.custom_check.completed,true);
  assert.equal(result.custom_check.totalMarks,0);assert.equal(result.custom_check.completed,true);
});

test('student formula zero with blank criteria remains incomplete',()=>{
  const {c}=setup({'Committee 1 - custom_check':[row('T1','A',Array(7).fill(''),0)]});
  const result=c.getStudentAllReviewMarks('a@example.com');
  assert.equal(result.custom_check.totalMarks,0);assert.equal(result.custom_check.completed,false);
});

test('marks joins normalize team and register identifiers across sheets',()=>{
  const {c}=setup({'Committee 1 - custom_check':[row(' t1 ',' a ',zero()),row('T1',' b ',zero())]});
  const result=c.getTeamReviewCompletionStatus_(' t1 ');
  assert.equal(result.teamId,'T1');assert.equal(result.custom_check.completed,true);
});

test('review timings preserve results and count opens and review reads',()=>{
 const f=setup({'Committee 1 - custom_check':[row('T1','A',zero()),row('T1','B',zero())]});
 ...
 assert.equal(timings.find(t=>t.phase==='review_detail_read').calls,2);
 assert.equal(timings.find(t=>t.phase==='review_read_lookup').calls,2);
